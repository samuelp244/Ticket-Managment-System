    ...
    <div>
      ...
      <div className="tickets_container">
        ...
        <div className="active_tickets">
          Active Tickets
          <hr />
          {tickets?.filter(obj=>obj.status === "Active").length === 0 ? (<p>none found</p>):}
        ...
        <div className="accepted_tickets">
          Accepted Tickets
          <hr />
          {tickets?.filter(obj=>obj.status === "Accepted").length===0 ? (<p>none found</p>):
          <ul>{tickets?.filter(obj=>obj.status === "Accepted").map((val)=>
          <div>Company-{val.organizationName}<br/>
          Category-{val.category}<br/>
          query-{val.query}<br/>
          status-{val.status}
          <button onClick={()=>closeButtonHandler(val._id)}>close</button>
        ...
        <div className="closed_tickets">
          Closed Tickets
          <hr />
          {tickets?.filter(obj=>obj.status === "closed").length===0 ? (<p>none found</p>):
